    ...
    try:
        ...
        with st.spinner("Running PMS reconciliation..."):
            ...
            if has_stage1_data:
                ...
                # Populate all expected stage1 keys (empty DataFrames where no data)
                st.session_state.dataframes['stage1'] = {
                    'starting_positions': current_positions_df,
                    'final_positions': current_positions_df,  # Same for simple mode
                    'processed_trades': pd.DataFrame(),  # Empty - no trades in simple mode
                    'parsed_trades': pd.DataFrame(),  # Empty - no trades in simple mode
                    'bloomberg_map': pd.DataFrame()  # Empty - not needed
                }
